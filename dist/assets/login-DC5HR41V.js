import{k as f,f as w,l as y,c as r}from"./index-iCVUMj6Q.js";function S(l){const b="/dungeons-dragons-app/";l.innerHTML=`
  ...
  `;const a=l.querySelector("[data-login-form]"),s=a.querySelector('input[name="signup"]'),c=a.querySelector("[data-login-submit]"),o=()=>{!c||!s||(c.textContent=s.checked?"Registrati":"Accedi")};s==null||s.addEventListener("change",o),o(),a.addEventListener("submit",async h=>{var p;h.preventDefault();const n=new FormData(a),d=String(n.get("email")),g=String(n.get("password")),i=n.get("signup")==="on",u=()=>{r("Esiste già un account con questa email. Prova ad accedere.","error")},v=()=>{r("Registrazione completata. Conferma l'email prima di procedere con il login.","success")};try{const e=i?await f.auth.signUp({email:d,password:g}):await f.auth.signInWithPassword({email:d,password:g});if(e.error)throw e.error;if(i){const m=(p=e.data.user)==null?void 0:p.identities;if(Array.isArray(m)&&m.length===0){u();return}v(),a.reset(),o();return}const t=e.data.user;t&&(w({user:t}),await y(t),window.location.hash="#/characters")}catch(e){const t=String((e==null?void 0:e.message)||"");if(i&&/already\s+(registered|exists|in use)|user\s+already/i.test(t)){u();return}r(e.message??"Errore login","error")}})}export{S as renderLogin};
